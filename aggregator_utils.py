...
def aggregate_subquery_results(query: str) -> pd.DataFrame:
    """
    1) call Semantic Scholar for 'query'
    2) call Litmaps for 'query'
    3) unify results
    4) return single DataFrame
    """
    # a) semantic scholar
    ...
    # c) merge
    combined_df = pd.concat([s2_df, litmaps_df], ignore_index=True)
    logging.info(f"aggregate_subquery_results -> got {len(combined_df)} items for '{query}'")
    return combined_df
